    ...
    createIceServer = function(url, username, password) {
        ...
        if (urlParts[0].indexOf("stun") === 0) {
            ...
            if (webrtcDetectedVersion < 27) {
                ...
                if (turnUrlParts[1].indexOf("transport=udp") === 0) {
                    iceServer = {
                        'url': turnUrlParts[0],
                        'credential': password,
                        'username': username
    ...
    var instance = function(controller, settings) {
        ...
        this.rtcPeerConnection = function(configuration, peerId, cb) {
            ...
            for (var dc in self.DataChannels) {
                ...
                this.RTCDataChannels[dataChannel.name] = this.Connection
                    .createDataChannel(dataChannel.name, configuration.dataChannelConstraints);
                this.Connection.ondatachannel = function(event) {
                    ...
                    receiveChannel.onmessage = function(messageEvent) {
                        ...
                        if (typeof messageEvent.data === "string") {
                            ...
                            bm.extractMessage(messageEvent.data,
                                function(m) {
                                    var find = m.topic;
                                    subs.get(function(s) {
                                            return s.topic == find;
                ...
                dataChannel.onpublishbinary = function(topic, bytes, data) {
                    for (var p in self.PeerConnections) {
                        if (self.PeerConnections[p].RTCDataChannels[dataChannel.name].readyState === "open") {
                            ...
                            var blob = new XSockets.BinaryMessage(msg,
                                bytes,
                                function(result) {
                                    console.log(result);
                ...
                dataChannel.onpublish = function(topic, data) {
                    ...
                    for (var p in self.PeerConnections) {
                        if (self.PeerConnections[p].RTCDataChannels[dataChannel.name]
                            .readyState ===
                            "open")
                ...
                dataChannel.onpublishTo = function(id, topic, data) {
                    var message = new XSockets.Message(topic, data);
                    if (self.PeerConnections[id])
                        self.PeerConnections[id].RTCDataChannels[dataChannel.name].send(JSON.stringify(message));
                };
            };
        ...
        self.bind("answer",
            function(event) {
                ...
                self.PeerConnections[event.Sender].Connection
                    .setRemoteDescription(new RTCSessionDescription(JSON.parse(event.Message)));
            });
        self.bind("offer",
            function(event) {
                ...
                window.MediaStreamTrack.getSources(function(results) {
                    var sources = results.map(function(sourceInfo, i) {
                        return {
                            ...
                            label: sourceInfo.label || sourceInfo.kind + " - " + i
                        };
                    });
                    cb(sources);
                });
...
XSockets.MediaRecorder = (function() {
    var recorder = function(stream, options) {
        ...
        this.options = XSockets.Utils.extend(options || {},
        {
            ...
            recorderId: XSockets.Utils.guid()
        });
